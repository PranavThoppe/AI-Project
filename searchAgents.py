...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    # Greedy nearest-neighbor chain using manhattan distances (admissible and consistent)
    dist = 0
    current = position
    remaining = list(unvisited)
    while remaining:
        nearest = min(remaining, key=lambda c: util.manhattanDistance(current, c))
        dist += util.manhattanDistance(current, nearest)
        current = nearest
